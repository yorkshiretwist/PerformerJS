...
i++;}});P.Show(P.children(el)[shown]);},Loop:function(e){r=false;var el=P.findEventElement(e,'A');if(e.type=='click'||P.keyCode(e)==13){var cls=P.classNames(el);var loop=P.classParam(cls,"targetEl",P.getAttribute(el,'rel'));if(P.$(loop)){var i=0;var toshow=0;var nowshowing=0;var n=P.nodeName;var v=P.visible;var dbg=P.Debug;var h=P.Hide;P.forEach(P.children(P.$(loop)),function(child){if(n(child)&&v(child)){dbg('- Currently showing item '+i,'subfunction');nowshowing=i;}
...
P.Debug('- Showing item '+toshow,'subfunction');P.Show(P.children(P.$(loop))[toshow],"fadein");P.stopEvent(e);}}},Tooltip:function(e,cssClass){var el=P.eventElement(e);if(!cssClass){cssClass="performertooltip";}
...
if(P.Prototype||P.Scriptaculous){return el.hasClassName(cls);}},$:function(el){if(typeof el=='object'){return el;}
if(typeof el=='undefined'){return false;}
if(P.Prototype||P.Scriptaculous){return $(el);}},$$:function(cls){var els;if(P.Prototype||P.Scriptaculous){els=$$(cls);}
if(els&&els.length>0){return els;}else{return false;}},$F:function(el){var elid=P.identify(el);var fields;if(P.Prototype||P.Scriptaculous){fields=el.getElements();}
return fields;},nodeName:function(el){var nn=false;if(P.Prototype||P.Scriptaculous){nn=el.nodeName.toLowerCase();}
if(!nn||typeof nn=='undefined'||nn=='#document'){return false;}else{return nn;}},getElementsByAttribute:function(type,attr,value){var s;if(value){s=type+"["+attr+"='"+value+"']";if(P.Prototype||P.Scriptaculous){return document.getElementsBySelector(s);}}else{s=type+"["+attr+"]";if(P.Prototype||P.Scriptaculous){return document.getElementsBySelector(s);}}},getValue:function(el){if(P.Prototype||P.Scriptaculous){return el.getValue();}},setValue:function(el,value){if(P.Prototype||P.Scriptaculous){el.value=value;return el;}},up:function(el,filter){if(P.Prototype||P.Scriptaculous){return el.up(filter);}},serialize:function(el){if(P.Prototype||P.Scriptaculous){return el.serialize(true);}},update:function(el,html){if(P.Prototype||P.Scriptaculous){return el.update(html);}},insertAfter:function(el,html){if(P.Prototype||P.Scriptaculous){return new Insertion.After(el,html);}},increment:function(){P.Counter++;return P.Counter;},identify:function(el){if(typeof(el)=='string'){el=P.$(el);}
var id=P.getAttribute(el,'id');if(!id||id==''){id='anonymous_element_'+P.increment();P.setAttribute(el,'id',id);}
return id;},getAttribute:function(el,attr){if(P.Prototype||P.Scriptaculous){return el.readAttribute(attr);}},setAttribute:function(el,attr,val){if(P.Prototype||P.Scriptaculous){return el.writeAttribute(attr,val);}},removeAttribute:function(el,attr){return el.removeAttribute(attr);},remove:function(el){if(P.Scriptaculous){Effect.Fade(el,{duration:0.5});}
if(P.Prototype||P.Scriptaculous){return el.remove();}},children:function(el,selector){if(typeof selector!='undefined'){selector=selector.replace('-',' ');el=P.$$(selector);}
if(P.Prototype||P.Scriptaculous){return el.childElements();}},ancestors:function(el){if(P.Prototype||P.Scriptaculous){return el.ancestors();}},classNames:function(el){var cls=P.getAttribute(el,"class");if(cls&&cls.length>0){var classes=[];var names=cls.split(/\s+/);var i=names.length;while(i--){if(names[i].length>0){classes[classes.length]=names[i];}}
return classes;}else{return[];}},visible:function(el){if(typeof(el)!='object'){el=P.$(el);}
if(P.Prototype||P.Scriptaculous){return el.visible();}},addClassName:function(el,cls){if(typeof(el)!='object'){el=P.$(el);}
if(P.Prototype||P.Scriptaculous){return el.addClassName(cls);}},removeClassName:function(el,cls){if(typeof(el)!='object'){el=P.$(el);}
if(P.Prototype||P.Scriptaculous){return el.removeClassName(cls);}},className:function(el,cls){if(typeof(el)!='object'){el=P.$(el);}
el.className=cls;},disableContext:function(el){if(P.Prototype||P.Scriptaculous){P.bind(el,'contextmenu',function(e){e.stop();});}
...
if(P.Scriptaculous){if(!effect||P.Effects.PerformerIndexOf(effect)==-1){return el.hide();}else{if(effect=='slideup'){return Effect.Fade(el,{duration:0.5});}
if(effect=='slidedown'){return Effect.Appear(el,{duration:0.5});}
...
if(P.Scriptaculous){if(!effect||P.Effects.PerformerIndexOf(effect)==-1){return el.show();}else{if(effect=='slideup'){return Effect.Fade(el,{duration:0.5});}
if(effect=='slidedown'){return Effect.Appear(el,{duration:0.5});}
if(effect=='blinddown'){return Effect.BlindDown(el,{duration:0.5});}
if(effect=='blindup'){return Effect.BlindUp(el,{duration:0.5});}
if(effect=='fadein'){return Effect.Appear(el,{duration:0.5});}
if(effect=='fadeout'){return Effect.Fade(el,{duration:0.5});}}}
return false;},Request:function(targetPage,requestMethod,params,successFunction){if(P.Prototype||P.Scriptaculous){new Ajax.Request(targetPage,{method:requestMethod,parameters:params,onSuccess:successFunction});}},getRequestText:function(request){var text=request;if(request.responseText){text=request.responseText;}
return text;},eventElement:function(e){var targ;if(!e){e=window.event;}
if(e.target){targ=e.target;}
else if(e.srcElement){targ=e.srcElement;}
if(targ.nodeType==3){return targ.parentNode;}
return targ;},findEventElement:function(e,tag){var target;if(P.Prototype||P.Scriptaculous){return Event.findElement(e,tag);}},domLoaded:function(func){if(P.Prototype||P.Scriptaculous){P.bind(document,"dom:loaded",func);}},bind:function(el,event,func){if(P.Prototype||P.Scriptaculous){Event.observe(el,event,func);}},unBind:function(el,event,func){if(P.Prototype||P.Scriptaculous){Event.stopObserving(el,event,func);}},stopEvent:function(e){if(P.Prototype||P.Scriptaculous){e.preventDefault();e.stopPropagation();Event.stop(e);return false;}
return false;},stopPropagation:function(e){e.stopPropagation();return false;},getDimensions:function(el){var size,getsize;if(P.Prototype||P.Scriptaculous){return el.getDimensions();}
size={};return size;},setStyle:function(el,style){if(P.Prototype||P.Scriptaculous){return el.setStyle(style);}},animate:function(el,params,duration){var elid=P.identify(el);if(typeof(el)!='object'){el=P.$(elid);}
if(P.Scriptaculous){$(elid).morph(params,{duration:duration});}
if(P.Prototype){$(elid).setStyle(params);}},setupMorphProperties:function(props,cls){var param,params,i,j;var cp=P.classParam;if(P.Scriptaculous||P.Prototype){params='';for(i=0,j=props.length;i<j;i++){param=cp(cls,props[i],false);var name=props[i];name=name.replace('backgroundColor','background');name=name.replace('fontSize','font-size');name=name.replace('borderWidth','border-width');name=name.replace('lineHeight','line-height');param=P.fixCSSParam(name,param);params+=name+': '+param+'; ';}}else{params={};for(i=0,j=props.length;i<j;i++){param=cp(cls,props[i],false);if(param!==false){params[props[i]]=param.replace("px","");}}}

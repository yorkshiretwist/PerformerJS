...
var Performer =
{
    ...
    DetectLibrary: function() {
        ...
        } else if (window.jQuery && typeof window.jQuery != 'undefined' && $().jquery) {
    ...
    Page: function(el) {
        ...
        var selector = Performer.classParam(cls, "selector", "undefined");
        if (selector != "undefined") {
			selector = "#" + elid + "-" + selector;
			var children = Performer.children(el, selector);
		} else {
			var children = Performer.children(el);
		}
        ...
        if (children.length > pagesize) {
            ...
            var menu = Performer.BuildPageMenu(elid, page);
            Performer.insertAfter(el, menu);
			Performer.addClassName(Performer.$(elid + '-page' + startpage), "currentpage");
            var links = Performer.$$("#" + elid + "-pagelinks .pagerlink");
    ...
    BuildPageMenu: function(elid, page) {
        var menu = '<ul class="performer-pagination" id="' + elid + '-pagelinks">';
        for (var x = 1; x <= page; x++) {
            menu += '<li><a href="#' + elid + '-page' + x + '" class="pagerlink" id="' + elid + '-page' + x + '">' + x + '</a></li>';
        }
        menu += '</ul>';
        return menu;
    },
    ...
    ShowPage: function(e) {
        ...
        Performer.forEach(Performer.$$("#" + parts[0] + "-pagelinks .pagerlink"), function(el) {
        ...
        if (el && Performer.nodeName(el) && (e.type == 'click' || Performer.keyCode(e) == 13)) {
            ...
            if (tabGroup && Performer.$(tabGroup)) {
                ...
                for (var i = 0; i < tablinks.length; i++) {
                    if (Performer.hasClassName(tablinks[i], 'tabGroup-' + tabGroup))
    ...
    doHide: function(el, effect) {
        ...
        if (Performer.Prototype) {
            ...
            if (!effect || Performer.Effects.PerformerIndexOf(effect) == -1) {
                ...
                if (effect == 'slideup') return $(el).slideUp("normal");
                if (effect == 'slidedown') return $(el).slideDown("normal");
                if (effect == 'blinddown') return $(el).slideDown("normal");
                if (effect == 'blindup') return $(el).slideUp("normal");
                if (effect == 'fadein') return $(el).fadeIn("normal");
                if (effect == 'fadeout') return $(el).fadeOut("normal");
            }
    ...
    findEventElement: function(e, tag) {
        ...
        if (Performer.jQuery) {
            var target = Performer.eventElement(e);

...
var Performer =
{
	...
	DoListeners: function(el)
	{
		...
		Performer.Listeners(el,'toggler','Toggle','click,keyup');
		Performer.Listeners(el,'switcher','Switch','click,keyup');
		Performer.Listeners(el,'loader','Load','click,keyup');
		Performer.Listeners(el,'toggleloader','ToggleLoad','click,keyup');
		Performer.Listeners(el,'sizer','Size','click,keyup');
		Performer.Listeners(el,'tabber','Tab','click,keyup');
		...
		Performer.Listeners(el,'grouptoggler','GroupToggle','click,keyup');
		// hooker listeners
		Performer.Listeners(el,'hooker-click','Hooker','click,keyup');
		Performer.Listeners(el,'hooker-keyup','Hooker','keyup');
		Performer.Listeners(el,'hooker-keydown','Hooker','keydown');
	...
	CheckForm: function(e)
	{
		...
		fields.each(function(input){
			...
			{
			    var num = Form.Element.getValue(field).replace(",","");
	...
	Match: function(e)
	{
		...
		if (el.readAttribute && el.readAttribute('id') && el.readAttribute('name') && el.readAttribute('class'))
		{
			...
			if (notifyEl && $(notifyEl) && matchEl && $(matchEl))
			{
				// show the notifyEl
				$(notifyEl).show();
				$(notifyEl).removeClassName('hider');
				// get the value
				var val1 = el.value;
		...
		if (el.readAttribute && el.readAttribute('rel') && el.readAttribute('rev'))
		{
			...
			if ($(targetEl1) && $(targetEl2))
			{
				...
				if (!$(targetEl1).visible())
				{
					...
					$(targetEl1).show();
					$(targetEl1).removeClassName('hider');
					$(targetEl2).hide();
					$(targetEl2).addClassName('hider');
					r = true;
				} else {
					Performer.Debug('-> Show ' + targetEl2 + ', hide '+ targetEl1, 'subfunction');
					$(targetEl2).show();
					$(targetEl2).removeClassName('hider');
					$(targetEl1).hide();
					$(targetEl1).addClassName('hider');
					r = false;
				}
			}
		}
	...
	Reload: function(e)
	{
		...
		if (Performer.Reloaders.indexOf(el.id) == -1)
		{
			...
			if (el && delay && targetPage)
			{
				...
				Performer.Debug('-> Added \'' + el.id + '\' to Performer.Reloaders (now ' + Performer.Reloaders.length + ' items)', 'subfunction');

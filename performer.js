...
var Performer =
{
	...
	DoListeners: function(el)
	{
		...
		Performer.Listeners(el,'toggler','Toggle','click,keypress');
		Performer.Listeners(el,'switcher','Switch','click,keypress');
		Performer.Listeners(el,'loader','Load','click,keypress');
		Performer.Listeners(el,'toggleloader','ToggleLoad','click,keypress');
		Performer.Listeners(el,'sizer','Size','click,keypress');
		Performer.Listeners(el,'tabber','Tab','click,keypress');
		Performer.Listeners(el,'selector','Select','change,keypress');
		Performer.Listeners(el,'limiter','Limit','keyup');
		Performer.Listeners(el,'editor','Edit','click,keypress');
		Performer.Listeners(el,'uneditor','UnEdit','click,keypress');
		Performer.Listeners(el,'prompter','RemovePrompt','focus');
		Performer.Listeners(el,'prompter','CheckPrompt','blur');
		Performer.Listeners(el,'popper','Pop','click,keypress');
		Performer.Listeners(el,'passwordchecker','CheckPassword','keyup');
		Performer.Listeners(el,'matcher','Match','keyup');
		Performer.Listeners(el,'grouptoggler','GroupToggle','click,keypress');
		// hooker listeners
		Performer.Listeners(el,'hooker-click','Hooker','click,keypress');
		Performer.Listeners(el,'hooker-keypress','Hooker','keypress');
		Performer.Listeners(el,'hooker-change','Hooker','change');
		Performer.Listeners(el,'hooker-mouseover','Hooker','mouseover');
		Performer.Listeners(el,'hooker-mouseout','Hooker','mouseout');
		Performer.Listeners(el,'hooker-submit','Hooker','submit');
		Performer.Listeners(el,'hooker-focus','Hooker','focus');
		Performer.Listeners(el,'hooker-blur','Hooker','blur');
		// make nice titles
		if(typeof makeNiceTitles == 'function')
		{
		    makeNiceTitles();
		}
	},
	...
	Hooker: function(e)
	{
		...
		if (el.readAttribute && el.readAttribute('id') && el.readAttribute('class'))
		{
			// get the event type
			var t = e.type;
			// get the classes
			...
			{
				// function
				if (cls.match(/^func-/)){ func = cls.replace('func-', ''); }
			});
	...
	Toggle: function(e,c)
	{
		...
		if (el.readAttribute && el.readAttribute('rel') && (e.type == 'click' || e.keyCode == Event.KEY_RETURN))
		{
			...
			if ($(targetEl))
			{
				...
				if (!$(targetEl).visible())
				{
					Performer.Debug('Performer.Toggle -> Show ' + targetEl, 'subfunction');
					el.addClassName('toggleropen');
					Performer.Show(targetEl);
					r = true;
				} else {
					Performer.Debug('Performer.Toggle -> Hide ' + targetEl, 'subfunction');
	...
	Switch: function(e,c)
	{
		...
		var el = Event.findElement(e, 'A');
		// check the element has the required attribute and is a valid event trigger
		if (el.readAttribute && el.readAttribute('rel') && el.readAttribute('rev') && (e.type == 'click' || e.keyCode == Event.KEY_RETURN))	
		{
	...
	Reload: function(e)
	{
		...
		if (Performer.Reloaders.indexOf(el.id) == -1)
		{
			...
			if (el && delay && targetPage)
			{
				...
				Performer.Debug('Performer.Reload -> Added \'' + el.id + '\' to Performer.Reloaders (now ' + Performer.Reloaders.length + ' items): ' + delay + ' seconds', 'subfunction');
	...
	Hide: function(e)
	{
		...
		if (el && elid)
		{
		    if ($(elid).className.indexOf('hider') != -1){ el.addClassName('hider'); }
		    if ($(elid).className.indexOf('shower') != -1){ el.removeClassName('shower'); }
			Performer.Debug('Performer.Hide -> Hiding ' + el.id, 'subfunction'); 
			el.hide();
		} else {
	...
	Show: function(e)
	{
		...
		if (el)
		{
		    if ($(elid).className.indexOf('hider') != -1){ el.removeClassName('hider'); }
		    if ($(elid).className.indexOf('shower') != -1){ el.removeClassName('shower'); }
			Performer.Debug('Performer.Show -> Showing ' + el.id, 'subfunction'); 
			el.show();
		} else {
	...
	Edit: function(e)
	{
		...
		if (el.readAttribute && el.readAttribute('id') && el.classNames() && (e.type == 'click' || e.keyCode == Event.KEY_RETURN))
		{
			var targetPage;
			var inputType;
			// loop the classes
	...
	Pop: function(e)
	{
		...
		var el = Event.findElement(e, 'A');
		// check the element has the required attributes and is a valid event trigger
		if (el.readAttribute && el.readAttribute('href') && el.readAttribute('rel') && el.readAttribute('rev') && (e.type == 'click' || e.keyCode == Event.KEY_RETURN))
		{
	...
	Debug: function(str, status)
	{
	    if (Performer.Debugging)
	    {
	        if ($('performerjsoutput') == null)
	        {
				...
				bodyhtml = bodyhtml + '<p><a href="#" class="toggler" rel="performerjsdebugbox" style="background:#333;color:#FFF;padding:0.3em 0.6em;margin:0 0 0 1em;-moz-border-radius-topright:5px;-moz-border-radius-topleft:5px;-webkit-border-top-right-radius:5px;-webkit-border-top-left-radius:5px;border:0">Performer Debug</a></p>\n';
				bodyhtml = bodyhtml + '<div class="hider" style="background:#333;padding:0.5em" id="performerjsdebugbox"><div id="performerjsoutput" style="padding:0.3em;height:400px;overflow:auto;background:#FFF;"></div></div>';
